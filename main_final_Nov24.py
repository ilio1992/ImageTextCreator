...
def adjust_font_to_fit(draw, text, font_path, max_width, max_height, initial_font_size=40):
    """
    Adjusts the font size dynamically to fit text within the specified width and height.
    """
    ...
    # Return the smallest font if no suitable size found
    return ImageFont.truetype(font_path, size=10)


def write_text_on_image(img, text_data):
    """
    Draws wrapped and centered text on an image with dynamic font adjustment.

    Args:
        img: PIL Image object.
        text_data: List of tuples containing:
            - text: The string to be drawn.
            - font_path: Path to the font file.
            - font_size: Initial font size (used for adjustment).
            - position: Tuple (x, y) for the text's center position.
            - text_color: Color of the text.
    """
    draw = ImageDraw.Draw(img)

    for text, font_path, font_size, position, text_color in text_data:
        # Define the maximum area for text (80% width, 20% height of the image)
        max_width = img.size[0] * 0.8
        max_height = img.size[1] * 0.2

        # Adjust the font dynamically to fit within the area
        font = adjust_font_to_fit(draw, text, font_path, max_width, max_height, font_size)

        # Calculate the maximum characters per line based on font and max width
        char_width, _ = font.getsize("A")
        max_chars = int(max_width / char_width)

        # Wrap the text dynamically
        para = textwrap.wrap(text, width=max_chars)
        if not para:
            continue

        # Calculate total height of wrapped text for vertical centering
        total_height = sum(
            draw.textbbox((0, 0), line, font=font)[3] - draw.textbbox((0, 0), line, font=font)[1] for line in para)
        ...
        for line in para:
            ...
            x_position = position[0] - text_width // 2
